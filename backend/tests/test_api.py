...
def test_cancel_marks_processing_job_cancelled(client):
    redis_client.set("job:job-x", json.dumps({"status": "processing"}))
    res = client.delete("/api/documents/jobs/job-x")
    assert res.status_code == 200
    assert json.loads(redis_client.get("job:job-x"))["status"] == "cancelled"
...
def test_cancel_does_not_clobber_finished_job(client):
    ...
    redis_client.set(
        "job:job-y", json.dumps({"status": "complete", "result": {"text": "done"}})
    )
    res = client.delete("/api/documents/jobs/job-y")
    assert res.status_code == 200
    assert json.loads(redis_client.get("job:job-y"))["status"] == "complete"
