...
import pipeline.gemini as gemini
...
def test_empty_text_skips_api(monkeypatch):
    ...
    urlopen = mock.Mock()
    monkeypatch.setattr(gemini.urllib.request, "urlopen", urlopen)

    assert gemini.correct_ocr_text("   ") == "   "
...
def test_missing_key_returns_raw(monkeypatch):
    ...
    assert gemini.correct_ocr_text("teh cat sat") == "teh cat sat"


def test_happy_path_returns_cleaned(monkeypatch):
    monkeypatch.setenv("GEMINI_API_KEY", "test-key")
    payload = {"candidates": [{"content": {"parts": [{"text": "The cat sat."}]}}]}
    response = mock.MagicMock()
    response.__enter__.return_value.read.return_value = json.dumps(payload).encode()
    monkeypatch.setattr(
        gemini.urllib.request, "urlopen", mock.Mock(return_value=response)
    )

    assert gemini.correct_ocr_text("teh cat sat") == "The cat sat."
